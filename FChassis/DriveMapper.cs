...
namespace FChassis {
   public static class DriveMapper {
      ...
      public static void CreateRequiredSubfolders (string basePath, bool createIfMissing = true) {
         string fchassisPath = Path.Combine (basePath, "FChassis");
         ...
         if (!Directory.Exists (fchassisPath)) {
            if (!createIfMissing) {
               throw new InvalidOperationException (
                   $"Target folder must contain 'FChassis' subfolder. " +
         ...
         if (!Directory.Exists (samplesPath)) {
            if (!createIfMissing) {
               throw new InvalidOperationException (
                   $"Target folder must contain 'FChassis/Sample' subfolder. " +
         ...
         if (!Directory.Exists (dataPath)) {
            if (!createIfMissing) {
               throw new InvalidOperationException (
                   $"Target folder must contain 'FChassis/Data' subfolder. " +
      ...
      public static bool HasRequiredFolderStructure (string path) {
         try {
            string fchassisPath = Path.Combine (path, "FChassis");
            string samplesPath = Path.Combine (fchassisPath, "Sample");
            string dataPath = Path.Combine (fchassisPath, "Data");
