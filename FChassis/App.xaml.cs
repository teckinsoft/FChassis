...
public partial class App : Application {
   ...
   void InitializeLogging () {
      try {
         ...
         string appFolderPath = Path.Combine (localAppDataPath, "FChassis");

         // Ensure the directory exists
         Directory.CreateDirectory (appFolderPath);

         // Create the full path for the log file
         mLogFilePath = Path.Combine (appFolderPath, "FChassis_StartupLog.txt");

         // Write a header to a new log file each time the app starts
         File.WriteAllText (mLogFilePath, $"--- FChassis Startup Log [{DateTime.Now}] ---\n");
         ...
         File.AppendAllText (mLogFilePath, $"--- FChassis Startup Log [{DateTime.Now}] ---\n");
      }
   }
