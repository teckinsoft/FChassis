...
public class Workpiece : INotifyPropertyChanged {
   ...
   public void Align () {
      ...
      if (size.Y > size.X) Apply (Matrix3.Rotation (EAxis.Z, Geo.HalfPI));

      // If the flanges are protruding 'downward', then rotate the model by 180 
      // degrees about the X axis
      if (-mModel.Bound.ZMin < mModel.Bound.ZMax) Apply (Matrix3.Rotation (EAxis.X, Geo.PI));
      ...
      double dz = -pbound.ZMin;        // Bottom of the baseplane is at Z=0, and two flanges are down in -Z territory
   ...
   public bool DoAddHoles () {
      if (HoleCutsComplete) return false;
      ...
      foreach (var ep in mModel.Entities.OfType<E3Plane> ()) {
         foreach (var con in ep.Contours.Skip (1)) {
            ...
            // If the contour appears to be CCW w.r.t a negative X with positive Y or vice versa after projection for YNeg plane
            // OR converse of the above consition if YPos plane, shape has to be reversed
            if ((type == EType.Top && ep.Xfm.M11 * ep.Xfm.M22 < 0.0) || (type == EType.YNeg && ep.Xfm.M11 * ep.Xfm.M23 < 0.0) ||
               (type == EType.YPos && ep.Xfm.M11 * ep.Xfm.M23 > 0.0)) {
               if (shape.Winding == EWinding.CCW) shape.Reverse ();
            } else if (shape.Winding == EWinding.CW) shape.Reverse ();
      ...
      foreach (var ef in mModel.Entities.OfType<E3Flex> ()) {
         ...
         foreach (var con in ef.Trims) {
            var b2 = con.Bound;
            if (b2.XMin.EQ (bound.XMin, 0.01) || b2.XMax.EQ (bound.XMax, 0.01)) continue;
            if (b2.YMin.EQ (bound.YMin, 0.01) || b2.YMax.EQ (bound.YMax, 0.01)) continue;
            var shape = con.Clone ().Cleanup (threshold: 1e-3);
      ...
      foreach (var cut in Cuts) {
         ...
         if (cut.Kind == EKind.Hole && Utils.GetFlangeType (cut,
            MCSettings.It.PartConfig == PartConfigType.LHComponent ? GCodeGenerator.LHCSys : GCodeGenerator.RHCSys) == Utils.EFlange.Flex) {
   ...
   public bool DoTextMarking () {
      if (MarkingsComplete) return false;
      int cutIndex = Cuts.Count + 1;
   ...
   public void DoSorting () {
      ...
      for (int i = 0; i < mCuts.Count; i++) mCuts[i].SeqNo = i;
      var box = mBound;
      ...
      for (int i = 1; i < mCuts.Count; i++) {
         ...
         double xmid = (prevToolingEndPlusClearance.X + currToolingStartPlusClearance.X) / 2,
                ymin = box.YMin - clearance, ymax = box.YMax + clearance, zmax = box.ZMax + clearance;
   ...
   public bool DoCutNotchesAndCutouts () {
      ...
      foreach (var ep in mModel.Entities.OfType<E3Plane> ()) {
         ...
         (Point3 p1, Point3 p2) = Classify (ep) switch {
            EType.Top => (new Point3 (mb.XMin, pb.YMin, mb.YMax), new Point3 (mb.XMax, pb.YMax, mb.YMax)),
            _ => (new Point3 (mb.XMin, pb.YMin, pb.ZMin), new Point3 (mb.XMax, pb.YMin, pb.ZMax)),
         };
      ...
      foreach (var ef in mModel.Entities.OfType<E3Flex> ()) {
         ...
         Point2 pt2 = ef.Trims.First ().P1; Point3 pt3 = ef.Project (pt2).Pt;
      ...
      while (!done) {
         ...
         for (int i = 0; i < cuts.Count - 1; i++) {
            for (int j = i + 1; j < cuts.Count; j++) {
               ...
               if (t0 == null) break;
               if (t1 == null) continue;
               Tooling tm = t0.JoinTo (t1, Tooling.mNotchJoinableLengthToClose) ?? t1.JoinTo (t0, Tooling.mNotchJoinableLengthToClose);
               if (tm != null) {
                  cuts.Add (tm); cuts[i] = cuts[j] = null; done = false;
               }
            }
            if (t0 == null) continue;
         }
      ...
      foreach (var cut in cuts) {
         if (cut != null) {
            ...
            if (cut.Kind == EKind.Cutout) {
               if (!MCSettings.It.CutCutouts) continue;
               ...
               if (yNegFlexFeat) q = new Point3 (0.0, mBound.YMin - 10.0, mBound.ZMax + 10.0);
               if (Geom.GetToolingWinding (n, q, cutSegs) == Geom.ToolingWinding.CW) cut.Reverse ();
               cut.CutoutKind = Tooling.GetCutKind (cut);
            } else {
               if (!MCSettings.It.CutNotches) continue;
   ...
   static List<Pline> GetNotches (Bound2 b, Pline p) {
      ...
      foreach (var seg in p.Segs) {
         if (seg.IsCurved) { output.Add (seg.ToPline ()); continue; }
         if (seg.A.Y.EQ (b.YMin, E) && seg.B.Y.EQ (b.YMin, E)) continue;      // Bottom edge
         if (seg.A.Y.EQ (b.YMax, E) && seg.B.Y.EQ (b.YMax, E)) continue;      // Top edge
         if (seg.A.X.EQ (b.XMin, E) && seg.B.X.EQ (b.XMin, E)) continue;      // Left edge
         if (seg.A.X.EQ (b.XMax, E) && seg.B.X.EQ (b.XMax, E)) continue;      // Right edge
         output.Add (seg.ToPline ());
      }
      for (int i = output.Count - 1; i >= 0; i--) {
         Pline one = output[(i + output.Count - 1) % output.Count], two = output[i];
         if (one.P2.EQ (two.P1)) { one.Append (two); output.RemoveAt (i); }
      }
      return output;
   }
   #endregion
