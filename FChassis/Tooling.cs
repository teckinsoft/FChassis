...
public class Tooling {
   public Tooling (Workpiece wp, E3Thick ent, Pline shape, EKind kind) {
   ...
   void OffsetStartingTraceToE3PlaneRef () {
      ...
      if (i > 0) Traces.Skip (i).Concat (Traces.Take (i)).ToList ();
   }
   ...
   public double Perimeter {
      get {
         if (mPerimeter < 0.0) mPerimeter = mPerimeter = Segs.Sum (seg => seg.Curve.Length);
         return mPerimeter;
      }
   }
   public void DrawSegs (Color32 color, double height) {
      ...
      foreach (var (Curve, Vec0, Vec1) in Segs) {
         if (first) { pvs.Add ((new (Curve.Start, Vec0), true)); first = false; }
      ...
      foreach (var (pv, stencil) in pvs) {
         if (stencil) Lux.Draw (EDraw.Lines, [pv.Pt, pv.Pt + pv.Vec * height]);
      }
   }
      ...
      get {
         ...
         foreach (var (ent, pline0) in Traces) {
            ...
            foreach (var seg in pline.Segs) {
               ...
               if (prevpvb != null) {
                  if (pva.DistTo (prevpvb.Value) < mNotchJoinableLengthToClose && pva.DistTo (prevpvb.Value) > 1.0) {
               ...
               if (seg.IsCurved) {
                  ...
                  for (int i = 0; i < steps; i++) {
                     ...
                     PointVec ps = Project (ent, seg.GetPointAt (start)), pe = Project (ent, seg.GetPointAt (end));
   ...
   public void Reverse () {
      ...
      if (Segs.Count == 0) Segs = ExtractSegs.ToList ();
      else Segs = Geom.GetReversedToolingSegments (Segs);
   }
   ...
   public PointVec Project (E3Thick ent, Point2 pt) {
      ...
      if (ent is E3Plane ep) {
         ...
         if (vec.Opposing (pt3 - mid)) vec = -vec; // Get the 'outward facing' vector
