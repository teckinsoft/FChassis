...
Function CreateShortcuts
  ...
  ; Set context to 'all' for Start Menu (common for all users)
  SetShellVarContext all
  
  ; Check if Start Menu directory exists
  IfFileExists "$SMPROGRAMS\${APPNAME}\*.*" dir_exists dir_not_exists
  
dir_exists:
  !insertmacro LogMessage "Start Menu directory already exists: $SMPROGRAMS\${APPNAME}"
  ; Attempt to clean up existing shortcut
  Delete "$SMPROGRAMS\${APPNAME}\${APPNAME}.lnk"
  IfErrors cleanup_failed cleanup_success
cleanup_failed:
  !insertmacro LogError "Failed to clean up existing Start Menu shortcut: $SMPROGRAMS\${APPNAME}\${APPNAME}.lnk"
  Goto try_user_context
cleanup_success:
  !insertmacro LogMessage "Cleaned up existing Start Menu shortcut (if any)"
  Goto check_exe

dir_not_exists:
  ; Create Start Menu directory with error checking
  !insertmacro LogMessage "Creating Start Menu directory: $SMPROGRAMS\${APPNAME}"
  CreateDirectory "$SMPROGRAMS\${APPNAME}"
  IfErrors dir_failed dir_success
  
dir_failed:
  !insertmacro LogError "Failed to create Start Menu directory: $SMPROGRAMS\${APPNAME}"
  Goto try_user_context

dir_success:
  !insertmacro LogVar "Start Menu directory" "$SMPROGRAMS\${APPNAME}"
  Goto check_exe

try_user_context:
  !insertmacro LogMessage "Falling back to current user context for Start Menu"
  SetShellVarContext current
  ; Check if user Start Menu directory exists
  IfFileExists "$SMPROGRAMS\${APPNAME}\*.*" user_dir_exists user_dir_not_exists
  
user_dir_exists:
  !insertmacro LogMessage "User Start Menu directory already exists: $SMPROGRAMS\${APPNAME}"
  Delete "$SMPROGRAMS\${APPNAME}\${APPNAME}.lnk"
  IfErrors user_cleanup_failed user_cleanup_success
user_cleanup_failed:
  !insertmacro LogError "Failed to clean up existing user Start Menu shortcut: $SMPROGRAMS\${APPNAME}\${APPNAME}.lnk"
  Goto create_desktop
user_cleanup_success:
  !insertmacro LogMessage "Cleaned up existing user Start Menu shortcut (if any)"
  Goto check_exe

user_dir_not_exists:
  !insertmacro LogMessage "Creating user Start Menu directory: $SMPROGRAMS\${APPNAME}"
  CreateDirectory "$SMPROGRAMS\${APPNAME}"
  IfErrors user_dir_failed user_dir_success
user_dir_failed:
  !insertmacro LogError "Failed to create user Start Menu directory: $SMPROGRAMS\${APPNAME}"
  Goto create_desktop
user_dir_success:
  !insertmacro LogVar "User Start Menu directory" "$SMPROGRAMS\${APPNAME}"
  
check_exe:
  ; Check if target executable exists
  IfFileExists "$INSTDIR\Bin\FChassis.exe" exe_exists exe_missing
  
exe_exists:
  !insertmacro LogMessage "Target executable found: $INSTDIR\Bin\FChassis.exe"
  
  ; Create Start Menu shortcut (use empty icon to avoid extraction issues)
  !insertmacro LogMessage "Creating Start Menu shortcut: $SMPROGRAMS\${APPNAME}\${APPNAME}.lnk -> $INSTDIR\Bin\FChassis.exe"
  CreateShortCut "$SMPROGRAMS\${APPNAME}\${APPNAME}.lnk" "$INSTDIR\Bin\FChassis.exe" "" "" 0
  IfErrors startmenu_failed startmenu_success

startmenu_success:
  !insertmacro LogMessage "Start Menu shortcut created successfully"
  Goto create_desktop

startmenu_failed:
  !insertmacro LogError "Failed to create Start Menu shortcut"
  ; Retry with explicit icon index 0 from the exe
  !insertmacro LogMessage "Retrying Start Menu shortcut with explicit icon index..."
  CreateShortCut "$SMPROGRAMS\${APPNAME}\${APPNAME}.lnk" "$INSTDIR\Bin\FChassis.exe" "" "$INSTDIR\Bin\FChassis.exe" 0
  IfErrors startmenu_retry_failed startmenu_success
startmenu_retry_failed:
  !insertmacro LogError "Retry failed for Start Menu shortcut"
  ; If in 'all' context and shortcut creation failed, try user context as a final fallback
  ${If} $ShellVarContext == "all"
    !insertmacro LogMessage "Final fallback: Trying user context for Start Menu"
    SetShellVarContext current
    IfFileExists "$SMPROGRAMS\${APPNAME}\*.*" user_dir_exists_final user_dir_not_exists_final
user_dir_exists_final:
    !insertmacro LogMessage "User Start Menu directory already exists in final fallback: $SMPROGRAMS\${APPNAME}"
    Delete "$SMPROGRAMS\${APPNAME}\${APPNAME}.lnk"
    IfErrors user_cleanup_final_failed user_cleanup_final_success
user_cleanup_final_failed:
    !insertmacro LogError "Failed to clean up existing user Start Menu shortcut in final fallback: $SMPROGRAMS\${APPNAME}\${APPNAME}.lnk"
    Goto final_user_dir_success
user_cleanup_final_success:
    !insertmacro LogMessage "Cleaned up existing user Start Menu shortcut in final fallback (if any)"
    Goto final_user_dir_success
user_dir_not_exists_final:
    !insertmacro LogMessage "Creating user Start Menu directory in final fallback: $SMPROGRAMS\${APPNAME}"
    CreateDirectory "$SMPROGRAMS\${APPNAME}"
    IfErrors final_user_dir_failed final_user_dir_success
final_user_dir_failed:
    !insertmacro LogError "Failed to create user Start Menu directory in final fallback: $SMPROGRAMS\${APPNAME}"
    Goto create_desktop
final_user_dir_success:
    !insertmacro LogVar "User Start Menu directory (final fallback)" "$SMPROGRAMS\${APPNAME}"
    CreateShortCut "$SMPROGRAMS\${APPNAME}\${APPNAME}.lnk" "$INSTDIR\Bin\FChassis.exe" "" "" 0
    IfErrors final_startmenu_failed final_startmenu_success
final_startmenu_failed:
    !insertmacro LogError "Final attempt to create user Start Menu shortcut failed"
    Goto create_desktop
final_startmenu_success:
    !insertmacro LogMessage "User Start Menu shortcut created successfully in final fallback"
  ${EndIf}
  Goto create_desktop

create_desktop:
  ; Switch to current user context for Desktop shortcut
  SetShellVarContext current
  !insertmacro LogVar "Desktop directory" "$DESKTOP"
  CreateShortCut "$DESKTOP\${APPNAME}.lnk" "$INSTDIR\Bin\FChassis.exe" "" "" 0
  IfErrors desktop_failed desktop_success

desktop_success:
  !insertmacro LogMessage "Successfully created Desktop link: $DESKTOP\${APPNAME}.lnk"
  Goto shortcuts_done
