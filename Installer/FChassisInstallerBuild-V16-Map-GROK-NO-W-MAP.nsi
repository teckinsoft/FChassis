...
Function .onInit
  ...
  SetRegView 64 ; Set 64-bit registry view for initialization
  ...
  ${If} $LocalAppDataDir == ""
    ...
    !insertmacro LogError "Failed to read LOCALAPPDATA, using $TEMP"
  ${EndIf}
  ...
  ${If} $LogFileHandle == ""
    !insertmacro LogError "Failed to open log file: $LocalAppDataDir\FChassis_Install.log"
    DetailPrint "ERROR: Failed to open log file"
  ${Else}
    FileWrite $LogFileHandle "=== FChassis Installation Log ===$\r$\n"
    FileWrite $LogFileHandle "Started: [$(^Time)]$\r$\n"
    !insertmacro LogMessage "Log file opened: $LocalAppDataDir\FChassis_Install.log"
  ${EndIf}
