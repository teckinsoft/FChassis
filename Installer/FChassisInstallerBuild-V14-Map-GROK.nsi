...
Function .onInstSuccess
  ...
  ; ------------------------------------------------------------
  ; Create the drive mapping using subst in current session
  ; ------------------------------------------------------------
  !insertmacro LogMessage "Creating W: drive mapping..."
  
  ; Remove any existing mapping first
  nsExec::ExecToStack 'cmd /c subst W: /D >nul 2>&1'
  Pop $1
  Pop $2
  
  ; Create the mapping
  nsExec::ExecToStack 'cmd /c subst W: "$0" >nul 2>&1'
  Pop $1
  Pop $2
  ...
  ; ------------------------------------------------------------
  ; Set up user-level persistence for redundancy
  ; ------------------------------------------------------------
