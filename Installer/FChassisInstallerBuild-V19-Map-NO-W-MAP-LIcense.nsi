...
Function LicensePre
  ...
  ; Initialize LicenseAgree to 0 (disagree by default)
...
Function LicenseShow
  ...
  ; Add label
  ${NSD_CreateLabel} 0 0u 100% 40u "The FChassis License Agreement will now open in your default PDF viewer. Please review the agreement carefully before proceeding."
  Pop $Label
  
  ; Add radio buttons
  ${NSD_CreateRadioButton} 0 50u 100% 12u "I agree to proceed further"
  Pop $RadioAgree
  ${NSD_OnClick} $RadioAgree OnRadioAgree
  
  ${NSD_CreateRadioButton} 0 70u 100% 12u "I do not agree"
  Pop $RadioDisagree
  ${NSD_OnClick} $RadioDisagree OnRadioDisagree
  
  ; Set default selection to "I do not agree"
  ${NSD_Check} $RadioDisagree
  
  ; Open the PDF file
  !insertmacro LogMessage "Attempting to open license agreement PDF: ${FluxSDKDir}\FChassis-License-Agreement.pdf"
  IfFileExists "${FluxSDKDir}\FChassis-License-Agreement.pdf" pdf_exists pdf_missing
pdf_exists:
  ExecShell "open" "${FluxSDKDir}\FChassis-License-Agreement.pdf"
  IfErrors pdf_open_failed pdf_open_success
pdf_open_failed:
  !insertmacro LogError "Failed to open license agreement PDF: ${FluxSDKDir}\FChassis-License-Agreement.pdf"
  MessageBox MB_OK|MB_ICONEXCLAMATION "Failed to open the license agreement PDF. Please ensure a PDF viewer is installed and the file is accessible at ${FluxSDKDir}\FChassis-License-Agreement.pdf."
  Goto show_dialog
pdf_open_success:
  !insertmacro LogMessage "Successfully opened license agreement PDF"
  Goto show_dialog
pdf_missing:
  !insertmacro LogError "License agreement PDF not found: ${FluxSDKDir}\FChassis-License-Agreement.pdf"
  MessageBox MB_OK|MB_ICONEXCLAMATION "License agreement PDF not found at ${FluxSDKDir}\FChassis-License-Agreement.pdf. Please ensure the file exists."
  Abort
show_dialog:
  nsDialogs::Show
FunctionEnd
...
Function OnRadioAgree
  ...
  !insertmacro LogMessage "User selected: I agree to proceed further"
FunctionEnd
...
; _HaveSMDir:
  ...
  ; ; ---------- Create Desktop shortcut (current user desktop) ----------
; _CreateDesktop:
  ; SetShellVarContext current
...
Function .onInit
  ...
  StrCpy $LicenseAgree 0 ; Initialize license agreement to disagree
