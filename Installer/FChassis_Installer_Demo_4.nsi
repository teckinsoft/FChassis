...
Section "Install"
  ...
  ; Immediate mapping for current user session
  ${If} ${RunningX64}
    ExecWait '"$WINDIR\Sysnative\subst.exe" W: "$INSTDIR\Senior"' $0
    MessageBox MB_OK "It is in 64"
  ${Else}
    ExecWait '"$WINDIR\System32\subst.exe" W: "$INSTDIR\Senior"' $0
    MessageBox MB_OK "It is in 32"
  ${EndIf}

  ${If} $0 == "error"
    MessageBox MB_OK|MB_ICONEXCLAMATION "Could not map W: drive immediately.$\nYou may need to log off or restart."
  ${Else}
    ${If} ${FileExists} "W:\"
...
;--------------------------------
Section "Uninstall"
  ; Remove subst mapping (interactive user)
  ExecWait '"$WINDIR\System32\subst.exe" W: /D' $0
