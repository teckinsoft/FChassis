   ...
   public void Add (LogType type = LogType.Normal, string message = "") {
      ...
      if (this.paragraphs.Count >= Logger.LineMax) {
         ...
         foreach(var inline in firstLine)
            this.textBlock.Inlines!.Remove ((AvaloniaDocuments.Run)inline);
      ...
      // Add new entry
      string _type = " Normal";
      AvaloniaMedia.IBrush color = AvaloniaMedia.Brushes.Black;
      switch(type) { 
         case LogType.Info:
            _type = "   Info";
            color = AvaloniaMedia.Brushes.LightSeaGreen;
            break;

         case LogType.Warning:
            _type = "Warning";
            color = AvaloniaMedia.Brushes.Goldenrod; // For Yellow
            break;

         case LogType.Error:
            _type = "  Error";
            color = AvaloniaMedia.Brushes.Red;
            break;
      };

      LogInlines logInlines = new LogInlines ();
      AvaloniaDocuments.Run run = new AvaloniaDocuments.Run () {
         FontFamily = new AvaloniaMedia.FontFamily (Logger.FontFamily)
      };
      ...
      switch (type) {
         ...
         default:
            ...
            run = new AvaloniaDocuments.Run () {
               Text = _type,
               Foreground = color,
               FontFamily = new AvaloniaMedia.FontFamily (Logger.FontFamily),
               FontWeight = AvaloniaMedia.FontWeight.ExtraBold
            };
            addRun ();
